...
def build_mlp(
    ...
    with tf.variable_scope(scope):
        layer = tf.layers.dense(inputs=mlp_input, units=size, activation=tf.nn.relu)
...
class PG(object):
    ...
    def build_policy_network_op(self, scope="policy_network"):
        ...
        if self.discrete:
            ...
            action_means = build_mlp(self.observation_placeholder, self.action_dim, scope,
                                     self.config.n_layers, self.config.layer_size, None)
            log_std = tf.get_variable("log_std", shape=[1, self.action_dim ], dtype=tf.float32,
    ...
    def add_baseline_op(self, scope="baseline"):
        ...
        self.baseline = build_mlp(self.observation_placeholder, 1, scope, self.config.n_layers, self.config.layer_size)
        self.baseline_target_placeholder =  tf.placeholder(tf.float32, shape=(None,))

        loss = tf.losses.mean_squared_error(labels=self.baseline_target_placeholder, predictions=tf.squeeze(self.baseline), scope=scope)
        opt = tf.train.AdamOptimizer(self.lr)
        self.update_baseline_op =  opt.minimize(loss)
        #######################################################
        #########          END YOUR CODE.          ############
    ...
    def get_returns(self, paths):
        ...
        for path in paths:
            ...
            for r in reversed(rewards):
                last_r = r+self.config.gamma*last_r
                time_reward.append(last_r)
            returns =  np.array(time_reward[::-1])
            #######################################################
            #########          END YOUR CODE.          ############
    ...
    def calculate_advantage(self, returns, observations):
        ...
        #########   YOUR CODE HERE - 5-10 lines.   ############
        if self.config.use_baseline:
            b = self.sess.run(self.baseline, feed_dict={self.observation_placeholder:observations})
            b = b.squeeze()
            adv = adv - b
    ...
    def train(self):
        ...
        for t in range(self.config.num_batches):
            ...
            if (last_reward+improve)>avg_reward:
                remain = remain +1
                if remain>6:
                    break
            else:
